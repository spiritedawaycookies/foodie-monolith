...
import com.lcy.utils.CookieUtils;
import com.lcy.utils.APIJSONResult;
import com.lcy.utils.JsonUtils;
import com.lcy.utils.MD5Utils;
...
public class PassportController {
    ...
    public APIJSONResult register(@RequestBody UserBO userBO,
        ...
        if (password.length() < 6) {
            return APIJSONResult.errorMsg("密码长度不能少于6");
        }

        // 3. 判断两次密码是否一致
        if (!password.equals(confirmPwd)) {
            return APIJSONResult.errorMsg("两次密码输入不一致");
        }
    ...
    public APIJSONResult login(@RequestBody UserBO userBO,
        ...
        return APIJSONResult.ok(userResult);
    }
